...
def test_embed_serves_trusted_spa_with_a_prefix_and_polling_shim():
    ...
    html = F._embed_html("body-1", "NONCE123").decode()
    assert "govd_monitor_token" in html                       # it IS the real local-monitor SPA
    assert '"/embed/body-1"' in html and "window.fetch=function" in html   # path-prefix shim
...
def test_render_node_iframe_live_vs_offline():
    node = {"name": "body-1", "role": "body", "url": "http://x:5773"}
    live = F.render_node_iframe(node, reachable=True)
    assert '<iframe src="/embed/body-1/?token=proxied"' in live and "/mnode/body-1" in live
    off = F.render_node_iframe(node, reachable=False)
    assert "offline" in off and "<iframe" not in off and "/mnode/body-1" in off   # falls back to the mirror board
