...
def _example_vars(sk, pk):
    d = registry.skill_dir(sk)
    meta = json.load(open(f"{d}/perks/{pk}/metadata.json"))
    ex = dict(meta.get("minimal_example", {}).get("vars", {}))
    contract = json.load(open(f"{d}/perks/{pk}/src/contracts.json"))
...
def test_perk_compiles_to_a_clean_script(sk, pk):
    ...
    man = json.load(open(f"{registry.skill_dir(sk)}/perks/{pk}/manifesto.json"))
...
def test_perk_declares_an_output_check(sk, pk):
    c = json.load(open(f"{registry.skill_dir(sk)}/perks/{pk}/src/contracts.json"))
...
def test_skill_blueprint_oversight_invariant_is_real(sk):
    ...
    bp = json.load(open(f"{registry.skill_dir(sk)}/blueprint.json"))
