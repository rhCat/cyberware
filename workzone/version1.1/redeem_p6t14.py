#!/usr/bin/env python3
"""redeem_p6t14.py — drive cws-settle-sim/adapter through a FRESH govd, then redeem P6-T14 onto done-ledger-v2.

P6-T14 (Stripe + internal-credits SettlementAdapter) is validated_by=cws-settle-sim, validated_perk=adapter — the perk built FOR
...
def main():
    ...
    try:
        ...
        rstore = os.path.join(RUNS, "redeem-p6t14", "run")
        ...
        ev_dir = os.path.join(RUNS, "redeem-p6t14", "evidence")
        os.makedirs(ev_dir, exist_ok=True)
        ev = os.path.join(ev_dir, "run-ledger.json")
        with open(ledger) as f, open(ev, "w") as g:
            g.write(f.read())
        rstore2 = os.path.join(RUNS, "redeem-p6t14", "redeem")
