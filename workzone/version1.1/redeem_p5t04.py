#!/usr/bin/env python3
"""redeem_p5t04.py — drive cws-chaos/failover through a FRESH govd, then redeem P5-T04 onto done-ledger-v2.

P5-T04 (HA active-passive govd + advisory-lock lease) is validated_by=cws-chaos, validated_perk=failover — the
perk built FOR this deliverable. Its hermetic HA selftest proves the three acceptance criteria: split_brain
(the lease is mutually exclusive; a partitioned second writer is gated + recorded), failover_drill (the active
dies, the standby acquires past the TTL, a re-sent step is idempotent — zero duplicate grants, zero lost
step_results), and no_orphaned_run (the interrupted run reconciles to a resumable, non-orphaned state). A clean
governed pass is the run-ledger the redeem binds (skill+perk) + chains onto done-ledger-v2. A fresh govd is
required so the blessed hashes match the just-re-pinned chip.
"""
import json
import os
import subprocess
...
def main():
    ...
    try:
        ...
        rstore = os.path.join(RUNS, "redeem-p5t04", "run")
        ...
        ev_dir = os.path.join(RUNS, "redeem-p5t04", "evidence")
        os.makedirs(ev_dir, exist_ok=True)
        ev = os.path.join(ev_dir, "run-ledger.json")
        with open(ledger) as f, open(ev, "w") as g:
            g.write(f.read())
        rstore2 = os.path.join(RUNS, "redeem-p5t04", "redeem")
