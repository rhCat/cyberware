#!/usr/bin/env python3
"""redeem_p1t06.py — drive cws-ledgercheck through a FRESH govd, then redeem P1-T06 onto done-ledger-v2.

P1-T06 (plan as sole source of step truth) is validated_by=cws-ledgercheck (validated_perk None, so the
redeem binds on the validator SKILL). cws-ledgercheck/principal is the same self-contained ledger-integrity
perk that evidenced P1-T08; a clean governed pass is the run-ledger the redeem chains onto done-ledger-v2.
...
def main():
    ...
    try:
        ...
        rstore = os.path.join(RUNS, "redeem-p1t06", "run")
        ...
        ev_dir = os.path.join(RUNS, "redeem-p1t06", "evidence")
        os.makedirs(ev_dir, exist_ok=True)
        ev = os.path.join(ev_dir, "run-ledger.json")
        with open(ledger) as f, open(ev, "w") as g:
            g.write(f.read())
        rstore2 = os.path.join(RUNS, "redeem-p1t06", "redeem")
