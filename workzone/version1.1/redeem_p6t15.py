#!/usr/bin/env python3
"""redeem_p6t15.py — drive cws-settle-sim/adapter through a FRESH govd, then redeem P6-T15 onto done-ledger-v2.

P6-T15 (Stripe + internal-credits dust account + adapter-boundary rounding) is validated_by=cws-settle-sim, validated_perk=adapter — the perk built FOR
...
def main():
    ...
    try:
        ...
        rstore = os.path.join(RUNS, "redeem-p6t15", "run")
        ...
        ev_dir = os.path.join(RUNS, "redeem-p6t15", "evidence")
        os.makedirs(ev_dir, exist_ok=True)
        ev = os.path.join(ev_dir, "run-ledger.json")
        with open(ledger) as f, open(ev, "w") as g:
            g.write(f.read())
        rstore2 = os.path.join(RUNS, "redeem-p6t15", "redeem")
