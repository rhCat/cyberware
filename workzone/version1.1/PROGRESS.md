...
*Snapshot: 2026-06-18.* A point-in-time review of the v1.1 build. Every claim below is checkable — nothing
is asserted that the machinery cannot redeem. The **live** board is [`pm-report.md`](pm-report.md)
(regenerated by `cws-pm` each update); this is the narrative around it. This snapshot is **chain-verified**:
`cws-observe/status` re-verified the v1+v2 done-ledger prev-hash chain **ok** at **65/90** with the ouroboros
passing.

*Recent engineering (not new redemptions — infra under the same governed bar).* The skillChip became a
**multi-source cartridge** (`cws/` + `general/`, future `nvidia/` / `claude/`): one resolver,
`registry.skill_dir`, maps a skill name to its dir whatever the layout, with the root `index.json` manifest
still the authoritative load set. A 16-agent adversarial audit of the move found 5 real issues (5 dismissed),
all fixed-forward — including a name-validation guard and an untracked-file authenticity gate on the governed
run. Landed across both repos, CI green ([cyberware#80](https://github.com/rhCat/cyberware/pull/80),
[#81](https://github.com/rhCat/cyberware/pull/81)).
...
redeem. SV-5 closed (8/8) with three independent provers (TLC + Apalache + TLAPS). **65 of 90 tasks** are
redeemed (each by a governed validator run recorded in the prev-hash-chained done-ledger, never asserted —
the live board is [`pm-report.md`](pm-report.md); `cws-observe/status` re-verifies the chain). M3's one open
brick is a hardware ceiling — the microVM perf budget needs `/dev/kvm`, absent on the dev box (see
[`KNOWN-BLOCKERS.md`](KNOWN-BLOCKERS.md) B1).
...
65 of 90 tasks, on the prev-hash-chained `cyberware-swarm-v1.1` done-ledger (v1 + the v2 continuation chain,
cross-referenced and re-verified each round). The complete task-by-task picture + the M0–M6 milestone closures
are the **live** board ([`pm-report.md`](pm-report.md)); re-verify the chain with `cws-observe/status`
(pass it the **v1** `done-ledger.json` — it auto-discovers the sibling v2). Highlights:
...
- **M3's last 1/10 is a hardware ceiling — the one open milestone.** P2-T09's microVM budgets need `/dev/kvm`
  + a microVM backend, absent on Docker Desktop/macOS (0 virt-capable CPUs). `bench_microvm()` reports
  `skipped`, never faked — full closure waits for a KVM-capable Linux host. See
  [`KNOWN-BLOCKERS.md`](KNOWN-BLOCKERS.md).
- **The other 25 of 90 are open work, not blocked closure.** M0/M1/M2/M4/M5/M6 are all **closed** (the SV
  ladder is up); every validator the plan references is **built** (`blocked:validator` = 0). What remains is
  16 ready-to-pull tasks and 9 blocked only on upstream dependencies (the live board, §-by-§, is
  [`pm-report.md`](pm-report.md)) — feature depth on a closed spine, not a missing rung.
