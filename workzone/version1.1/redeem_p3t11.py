#!/usr/bin/env python3
"""redeem_p3t11.py — drive cws-release/tierwire through a FRESH govd, then redeem P3-T11 onto done-ledger-v2.

P3-T11 (tier wiring to the P2 sandbox profiles) is validated_by=cws-release, validated_perk=tierwire — the perk
built FOR this deliverable. Its hermetic validator proves both acceptance criteria: `tier_enforced_at_grant`
(the grant's sandbox_tier selects the confinement backend through exod.run_step, monotone over the operator
floor) and `community_no_secrets_schema_and_runtime` (the P2-T04 no-secrets floor at schema + runtime). A clean
...
def main():
    ...
    try:
        ...
        rstore = os.path.join(RUNS, "redeem-p3t11", "run")
        ...
        ev_dir = os.path.join(RUNS, "redeem-p3t11", "evidence")
        os.makedirs(ev_dir, exist_ok=True)
        ev = os.path.join(ev_dir, "run-ledger.json")
        with open(ledger) as f, open(ev, "w") as g:
            g.write(f.read())
        rstore2 = os.path.join(RUNS, "redeem-p3t11", "redeem")
