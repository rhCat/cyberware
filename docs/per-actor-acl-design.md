...
> **Post-namespace addendum (Steps 1–6).** Since this was written, skills became **namespaced** — the canonical
> id is now `<namespace>:<name>` (e.g. `general:fs`, `cws:cws-observe`), not the bare `cws-fs`/`cws-git` forms
> used in the examples below. Three things changed in the ACL: (1) `principals.acl_allows` now matches a skill
> via `_skill_listed`/`_pmap_entry`, which accept an exact `ns:name`, a **per-namespace `ns:*` wildcard**, the
> `*` super-wildcard, **or a legacy bare leaf** (a pre-cutover entry `fs` still admits the single-owner
> canonical claim `general:fs`) — so the `skill in skills` / `skill in pmap` pseudocode below is the
> pre-namespace form. (2) This per-actor ACL (now **ACCESS-2**) sits beside a new, independent **ACCESS-1**
> gate — the skill's *own* `access.json` policy (`infra/govern/skillacl.py`, local-open/remote-closed); its
> `skillacl_sha` is recorded **separately** from `acl_sha` (folding it into `acl_sha` would break exod's
> acl_join). See [architecture.md](architecture.md) "The three govern gates". (3) The "Catalog filtering …
> Bearer-gate + scope-filter `GET /catalog`" noted further down as pulled into the MVP is **not built** —
> `/catalog` is still served **ungated** (`govd.py`), matching governance-service.md; treat it as Phase-2, not
> shipped.
...
6. PARAMS-axis upgrade (v1.3, the delegated var-value channel): folding `params` into acl_canonical CHANGES
   acl_sha for EVERY actor (a params-less actor now canonicalizes `params: null`), so an operator ACL
   attestation minted under the pre-params code fails exod's self-consistency re-derive (acl_sha_mismatch)
   on an upgraded node. Bounded: it bites only where ACL M1 is provisioned (acl-issuer.pub pinned), only
   REFUSES under --acl-strict (default audit logs "would refuse"), and attestation TTLs are minutes-to-hours
