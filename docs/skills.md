...
| **cws-fleet** | `status` · `deploy` · `down` | python3 · docker | govern the fleet + give a subagent a **scoped governance domain**: `status` is a read-only value-free fleet overview; `deploy` composes a least-privilege subset chip (cartridge), mints a scoped principal, and spins the subagent its OWN govd at a **strictly-lower `fleet_tier`** (content-identity subset of the parent — a trojaned same-name skill can't ride along); `down` tears it down (durable mirror preserved). Token via `*_FILE`; `CONFINE=1` requires a delegated+exod body. Needs a running daemon. |
| **cws-provision** | `up` | bash | bring a node up as a **cyberware fleet member** in one governed run — base deps, Docker, Tailscale (private-overlay join), and the **always-on syscontrol** (enable + `Restart=always` so services survive a reboot). Governed-clean (apt, never `curl\|sh` / `sudo`); operator-run as root; `DRY_RUN=1` plans only. Destructive (host-modifying). |
