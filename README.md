...
- **per-skill** — every one of the 27 perks must compile to a clean, consistent, oversight-clear script
  (contract tests), and the hermetic skills run **end-to-end through the governed executor** on fixtures
  (service-dependent ones are `skipif`-guarded).

```sh
pytest tests          # ~165 tests, a few seconds
```
